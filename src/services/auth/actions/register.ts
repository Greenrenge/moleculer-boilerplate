...
import type { IToken } from '@auth/utils/token'
import { issueUserAccessToken } from '@auth/utils/token'
...
export default {
	...
	handler: async function register(ctx: AppContextMeta<RegisterParams>): Promise<IToken> {
		...
		const userPublicProfile = await this.broker.call('v2.user.create', userInfo, {
			meta: {
				...ctx.meta,
				userId: _id,
			},
