...
if [[ -z $1 ]]; then
    ...
    docker compose -p local_mongors up  -d --build -V --remove-orphans
else
    echo "Composing $1"
...
docker exec -i mongoset1 bash < ./scripts/rs-init.sh
docker compose restart
sleep 30
echo "Running root and admin users ..."
# docker exec mongodb /scripts/user-init.sh
docker exec -i mongoset1 bash < ./scripts/user-init.sh
